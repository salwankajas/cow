...
export default function DisplayEvents(props: { name: string, poster: string, age: number, insurance: string, mark: string, breed: string, id: string, checked?: boolean, content?: { descr: string, doctor: string, time: string }, uid: string }) {
        ...
        <>
            <div className="my-24 text-white">
                <div className="w-11/12 mx-auto min-h-[24rem] my-12 bg-[rgba(57,176,255,0.6)] rounded-lg pb-3 shadow-lg md:w-8/12">
                    ...
                    <>
                        <div className="bg-gray-600 w-60 rounded-lg relative mx-auto bottom-[2rem] overflow-hidden shadow-2xl"><LazyImage src={props.poster} alt="poster" width="0" height="0" sizes="100vw" className="w-full h-auto" /></div>
                        ...
                        {trash ? <Trash popOff={popOff} id={props.uid} animal={props.insurance} /> : ""}
                    </>
                    {/* } */}
                </div>
            </div>

...
import {readFarmer} from '../../app/dashboard/api/read';
...
export default function Events(){
    ...
    return(
        ...
        <DisplayFarmer name={"sfs"} email={"Sdf"} key={1} adhaar={"DSfsdf"}/>] */}
            ...
            data.forEach((ele,i)=>{
                const data = ele.data();
                arr.push(<DisplayFarmer name={data.name} email={data.email} key={i} adhaar={data.adhaar} id={ele.id} />)
            })
            // data.docs.map((ele,i) => {
            //     const data = ele.data()
